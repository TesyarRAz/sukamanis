            ...
            @foreach ($beritas as $item)
                <div class="col">
                    <div class="card border-0 shadow-sm h-100 rounded-4 overflow-hidden berita-card">
                        <img src="{{ $item->getFirstMediaUrl('thumbnail') }}" class="card-img-top berita-img"
                            alt="gambar berita">
                        <div class="card-body">
                            <p class="fw-semibold">{{ $item->headline }}</p>
                            <a href="{{ route('beritashow', $item->slug) }}" class="ms-1 stretched-link"></a>
                        </div>
                    </div>
                </div>
            @endforeach
            ...
            @foreach ($galleries as $item)
                <div class="col">
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden gallery-card">
                        <img src="{{ $item->getFirstMediaUrl('gallery') }}" class="card-img-top"
            ...
            @foreach ($aparaturs as $item)
                <div class="col">
                    <div class="card border-0 shadow-sm p-3 rounded-4 aparatur-card">
                        <img src="{{ $item->getFirstMediaUrl('photo') }}" class="rounded-circle mx-auto"
                            style="width:100px; height:100px; object-fit:cover;" alt="gambar aparatur">
                        <div class="card-body">
